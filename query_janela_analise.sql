/* essa query está com a produtividade pelo datetime real */
...
janelas_expandidas AS (
    SELECT
        ...
        -- Cálculo da duração da janela programada de received em minutos
        CASE
            WHEN TRY_CAST(REPLACE(jp.labeling_start_time,':','') AS INT) > TRY_CAST(REPLACE(jp.labeling_end_time,':','') AS INT) THEN
                -- Caso a janela cruze a meia-noite
                (24 * 60 - (FLOOR(TRY_CAST(REPLACE(jp.labeling_start_time,':','') AS INT) / 100) * 60 + TRY_CAST(REPLACE(jp.labeling_start_time,':','') AS INT) % 100)) +
                (FLOOR(TRY_CAST(REPLACE(jp.labeling_end_time,':','') AS INT) / 100) * 60 + TRY_CAST(REPLACE(jp.labeling_end_time,':','') AS INT) % 100)
            ELSE
                -- Caso a janela esteja no mesmo dia
                (FLOOR(TRY_CAST(REPLACE(jp.labeling_end_time,':','') AS INT) / 100) * 60 + TRY_CAST(REPLACE(jp.labeling_end_time,':','') AS INT) % 100) -
                (FLOOR(TRY_CAST(REPLACE(jp.labeling_start_time,':','') AS INT) / 100) * 60 + TRY_CAST(REPLACE(jp.labeling_start_time,':','') AS INT) % 100)
        END AS duracao_minutos_janela_programada_received
...
operadores_dispatch_agrupado AS (
    SELECT
        pd.station_id,
        pd.data_base_janela,
        pd.janela_dispatch_expandida,
        COUNT(DISTINCT o.operator_name) AS total_distinto_operadores_dispatch
    FROM percentuais_janela_dispatch pd
    LEFT JOIN operadores_janela o
        ON pd.station_id = o.station_id
        AND o.data_base_janela = pd.data_base_janela
        AND o.datetime_received BETWEEN 
            ...
            AND 
            CASE 
                WHEN CAST(REGEXP_EXTRACT(pd.janela_programada_dispatch, '^(\\d{2}):', 1) AS INTEGER) > CAST(REGEXP_EXTRACT(pd.janela_programada_dispatch, '(\\d{2}):\\d{2}$', 1) AS INTEGER) THEN
                    CAST(pd.data_base_janela + INTERVAL '1' DAY AS TIMESTAMP) + INTERVAL '1' HOUR * CAST(REGEXP_EXTRACT(pd.janela_programada_dispatch, '(\\d{2}):\\d{2}$', 1) AS INTEGER) + INTERVAL '1' MINUTE * CAST(REGEXP_EXTRACT(pd.janela_programada_dispatch, '(\\d{2}):(\\d{2})$', 2) AS INTEGER)
                ELSE
            ...
            END
    WHERE o.operator_name IS NOT NULL
    GROUP BY pd.station_id, pd.data_base_janela, pd.janela_dispatch_expandida
)
...
SELECT DISTINCT
    ...
    ROUND(CAST(je.duracao_minutos_janela_programada_received AS DOUBLE) / 60, 2) AS duracao_horas_janela_programada_received,
    ...
    CAST(FLOOR((CAST(p.total_da_janela_received AS DOUBLE) / NULLIF(c.total_minutos_real_received, 0)) * 60) AS INTEGER) AS produtividade_received,
    o.total_distinto_operadores AS total_operadores_received,
    CAST(FLOOR(CAST(p.total_da_janela_received AS DOUBLE) / NULLIF(o.total_distinto_operadores, 0)) AS INTEGER) AS produtividade_por_operador_received,
    ...
    CAST(FLOOR((CAST(pd.total_da_janela_dispatch AS DOUBLE) / NULLIF(cd.total_minutos_real_dispatch, 0)) * 60) AS INTEGER) AS produtividade_dispatch,
    (
    ...
    CAST(FLOOR(CAST(pd.total_da_janela_dispatch AS DOUBLE) / NULLIF((
        SELECT COUNT(DISTINCT o2.operator_name)
        FROM operadores_janela o2
        WHERE o2.station_id = pd.station_id
        AND o2.data_base_janela = pd.data_base_janela
        AND o2.datetime_received BETWEEN 
            ...
            AND 
            CAST(pd.data_base_janela AS TIMESTAMP) + INTERVAL '1' HOUR * CAST(REGEXP_EXTRACT(pd.janela_programada_dispatch, '(\d{2}):\d{2}$', 1) AS INTEGER) + INTERVAL '1' MINUTE * CAST(REGEXP_EXTRACT(pd.janela_programada_dispatch, '(\d{2}):(\d{2})$', 2) AS INTEGER)
    ), 0)) AS INTEGER) AS produtividade_por_operador_dispatch,
    c.total_minutos_real_received AS total_minutos_real_received_debug
FROM percentuais_janela p
...
LEFT JOIN operadores_dispatch_agrupado od
    ON pd.station_id = od.station_id
    AND pd.data_base_janela = od.data_base_janela
    AND pd.janela_dispatch_expandida = od.janela_dispatch_expandida
...
GROUP BY 
    ...
    cd.duracao_curva_dispatch,
    c.total_minutos_real_received,
    od.total_distinto_operadores_dispatch,
    je.duracao_minutos_janela_programada_received

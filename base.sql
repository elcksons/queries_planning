...
curva_5_percent AS (
    SELECT
        ...
        MIN(CASE WHEN percentual_janela > 0.05 THEN datetime_received END) AS datetime_inicio_curva_received,
        MAX(CASE WHEN percentual_janela > 0.05 THEN datetime_received END) AS datetime_fim_curva_received,
        date_diff('minute', 
            MIN(CASE WHEN percentual_janela > 0.05 THEN datetime_received END),
            MAX(CASE WHEN percentual_janela > 0.05 THEN datetime_received END)
        ) AS duracao_curva
...
curva_5_percent_dispatch AS (
    SELECT
        ...
        MIN(CASE WHEN percentual_janela_dispatch > 0.05 THEN datetime_delivering END) AS datetime_inicio_curva_dispatch,
        MAX(CASE WHEN percentual_janela_dispatch > 0.05 THEN datetime_delivering END) AS datetime_fim_curva_dispatch,
        date_diff('minute', 
            MIN(CASE WHEN percentual_janela_dispatch > 0.05 THEN datetime_delivering END),
            MAX(CASE WHEN percentual_janela_dispatch > 0.05 THEN datetime_delivering END)
...
GROUP BY 
    ...
    cd.duracao_curva_dispatch,
    c.total_minutos_real_received,
    CAST(FLOOR(CAST(p.total_da_janela_received AS DOUBLE) / NULLIF(o.total_distinto_operadores, 0)) AS INTEGER),
    CAST(FLOOR(CAST(pd.total_da_janela_dispatch AS DOUBLE) / NULLIF((
        SELECT COUNT(DISTINCT o2.operator_name)
        FROM operadores_janela o2
        WHERE o2.station_id = pd.station_id
    ...
    ROUND(CAST(c.total_minutos_real_received AS DOUBLE) / 60, 2),
    ROUND(CAST(cd.total_minutos_real_dispatch AS DOUBLE) / 60, 2)
